...
//enum SobelDisplayMode g_SobelDisplayMode = SOBELDISPLAY_SOBELTEX;
enum SobelDisplayMode g_SobelDisplayMode = SOBELDISPLAY_SOBELSHARED;
...
void keyboard( unsigned char key, int /*x*/, int /*y*/) 
{
   char temp[256];
	...
	case 'd':
    case 'D':
	    g_SobelDisplayMode = DISPLAY_DILATION;
	    sprintf(temp, "Cuda Dilation (%s)", filterMode[g_SobelDisplayMode]);
	    glutSetWindowTitle(temp);
	    break;
	case 'e':
    case 'E':
	    g_SobelDisplayMode = DISPLAY_EROSION;
	    sprintf(temp, "Cuda Erosion (%s)", filterMode[g_SobelDisplayMode]);
	    glutSetWindowTitle(temp);
	    break;
	case 'i': 
    case 'I':
...
void runAutoTest(int argc, char **argv)
{
    ...
    while (g_SobelDisplayMode <= DISPLAY_EROSION)
    {
...
*/
        ...
        if (!bQuit) {
            ...
            printf("D: display dilation image\n");
            printf("E: display erosion image\n");
